...
async function constructServer(moduleDefs) {
  const app = express()
  const pluginLoginResults = new Map()
  ...
  // One-time localhost handoff used by the plugin login page.
  app.post('/virtualdj_plugin/login/complete', (req, res) => {
    const session = String(req.body?.session || '')
    const cookie = String(req.body?.cookie || '')
    if (!session || !cookie || !['127.0.0.1', '::1', '::ffff:127.0.0.1'].includes(req.ip))
      return res.status(400).json({ code: 400, message: 'invalid login result' })
    pluginLoginResults.set(session, { cookie, expires: Date.now() + 10 * 60 * 1000 })
    res.set('Cache-Control', 'no-store').json({ code: 200 })
  })
  app.get('/virtualdj_plugin/login/result', (req, res) => {
    const session = String(req.query.session || '')
    const result = pluginLoginResults.get(session)
    res.set('Cache-Control', 'no-store')
    if (!result || result.expires < Date.now()) {
      if (result) pluginLoginResults.delete(session)
      return res.json({ code: 202 })
    }
    pluginLoginResults.delete(session)
    res.json({ code: 200, cookie: result.cookie })
  })
